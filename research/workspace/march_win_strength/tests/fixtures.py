            ...
            for i in range(len(ids)):
                for j in range(i+1,len(ids)):
                    ...
                    games.append({'Season':year,'DayNum':day,'WTeamID':winner,'LTeamID':loser,
                       'WLoc':{-1:'A',0:'N',1:'H'}[loc],'WScore':80,'LScore':70})
...
def build_fixture(root):
    root=Path(root);names={'kit':'march_win_strength','repo':'repo','shooting':'march_shooting_research',
        'schedule':'march_schedule_research','temporal':'march_temporal_form','possession':'march_possession_research'}
    args={k:root/n for k,n in names.items()}
    for p in args.values():p.mkdir(parents=True)
    kit,repo=args['kit'],args['repo']
    for n in w.SOURCES+['10_mens_win_strength.ipynb']:
