"""Milestone 12: shared-system ranking hypotheses, frozen tournament classifier."""
from __future__ import annotations
import argparse, hashlib, json, os, shutil, zipfile
from pathlib import Path
import numpy as np
import pandas as pd
import ranking_features as wf
...
SOURCES = ['ranking_features.py', 'ranking_workflow.py', 'research_io.py',
           'ranking_plots.py', 'run_round12.py',
           'frozen/research_workflow.py', 'frozen/shot_features.py']
CONFIG = {'round':'12-men-shared-ranking-matchups', 'genders':['M'], 'seasons':SEASONS,
    'train_first_season':2013,'recipes':wf.RECIPES,'parameters':wf.PARAMETERS,
    'C':.1,'threads':2,'fit_cap':16,'rating_fit_cap':0,
    'primary':'pairwise_given_consensus','secondary':'consensus_given_anchor',
...
PAIR_FILES = ['matchups.csv','team_profiles.csv','pair_support.csv','support.json']
PREP_FILES = ['prepare.json','coverage.csv','feature_registry.csv','prior_replay.csv',
 'team_profiles.csv','panel_coverage.csv','pair_support.csv','prior_results_check.json']
...
def read_previous_report(path, expected_sha):
    ...
    with zipfile.ZipFile(path) as z:
        ...
        require(len(metrics)==16 and not metrics.duplicated(['Season','recipe']).any(),'Previous comparison grid invalid')
        require(set(metrics.Season)==set(SEASONS) and set(metrics.recipe)=={'anchor','anchor_huber','anchor_compressed','anchor_both'},'Unexpected previous experiment')
...
def preflight(kit, repo, shooting, schedule, temporal, possession, margin):
    ...
    prior_zip = safe_file(paths['margin'], 'reports/milestone_11_return.zip')
    prior_metrics, prior_decisions, prior_hashes = read_previous_report(prior_zip, c['previous_report_sha256'])
    observed = read_csv(kit/'evidence/round11/metrics.csv')
    left=prior_metrics.set_index(['Season','recipe']).sort_index().brier
    right=observed.set_index(['Season','recipe']).sort_index().brier
    require(left.index.equals(right.index) and np.max(abs(left.to_numpy()-right.to_numpy()))<1e-12,'Scientific scores differ from uploaded report')
    ...
    require(shutil.disk_usage(out).free >= 256 * 1024**2, 'Under 256 MiB free; stop before fitting')
    ...
    event('preflight_pass', base_snapshots=7, reference_classifiers=4, rating_fit_cap=0, prior_report_verified=True)
...
def prepare(ctx):
    out = ctx['directory']
    missing = [s for s in YEARS if not checkpoint(out/f'panels/M_{s}', PANEL_FILES)]
    new_panels = 0
    if missing:
        # One chunked pass. Only legal editions and declared seasons are retained.
        legal = wf.read_legal_rankings(ctx['raw']/'MMasseyOrdinals.csv', missing)
        for season in missing:
            panel, support = wf.publication_panel(legal, season)
            folder = out/f'panels/M_{season}'
            atomic_csv(folder/'panel.csv', panel)
            atomic_json(folder/'panel_support.json', support)
            seal(folder, PANEL_FILES); new_panels += 1
            event('ranking_panel_complete', season=season, completed=new_panels, total=len(missing))
    built = 0; reused = 0; coverage = []; profiles = []; supports = []; panels = []
    for season in YEARS:
        pfolder = out/f'panels/M_{season}'
        require(checkpoint(pfolder, PANEL_FILES), 'Ranking panel checkpoint missing')
        panel = read_csv(pfolder/'panel.csv'); panels.append(read_json(pfolder/'panel_support.json'))
        base = read_csv(ctx['shooting']/f'snapshots/M_{season}/teams.csv')
        folder = out/f'snapshots/M_{season}'
        if checkpoint(folder, PAIR_FILES):
            reused += 1
        else:
            pairs, profile, support, detail = wf.build_matchups(base, panel)
            atomic_csv(folder/'matchups.csv', pairs); atomic_csv(folder/'team_profiles.csv', profile)
            atomic_json(folder/'support.json', support); atomic_csv(folder/'pair_support.csv', detail)
            seal(folder, PAIR_FILES); built += 1
        profiles.append(read_csv(folder/'team_profiles.csv'))
        coverage.append(read_json(folder/'support.json')); supports.append(read_csv(folder/'pair_support.csv'))
        event('matchup_features_complete', season=season, completed=len(coverage), total=len(YEARS), new_rating_fits=0)
    ...
    for season in SEASONS:
        ...
        require(abs(scores[season]-metric['brier']) < 1e-12, 'Reference differs from completed milestone 11')
        records.append({'Gender':'M', 'Season':season, 'brier':metric['brier'], 'status':'VERIFIED_REPLAY'})
    for n, frame in [('coverage.csv', pd.DataFrame(coverage)),('feature_registry.csv', wf.registry()),
        ('prior_replay.csv', pd.DataFrame(records)),('team_profiles.csv', pd.concat(profiles, ignore_index=True)),
        ('panel_coverage.csv', pd.DataFrame(panels)),('pair_support.csv', pd.concat(supports, ignore_index=True))]:
        atomic_csv(out/n, frame)
    atomic_json(out/'prior_results_check.json', {'status':'VERIFIED','previous_report_sha256':ctx['identity']['prior_report_sha256'],
        'uploaded_scores_reproduced':16, 'decisions':ctx['prior_decisions'], 'new_models_fitted_for_review':0})
    atomic_json(out/'prepare.json', {'status':'COMPLETE','base_snapshots_reused':7,'reference_replays':4,
        'new_rating_fits':0,'new_rank_panels':new_panels,'panel_reuses':len(YEARS)-new_panels,
        'new_matchup_tables':built,'matchup_table_reuses':reused,'new_candidate_features':2,
        'established_consensus_controls':1,'new_tournament_classifier_fits':0,'tournament_outcomes_used_for_feature_building':False})
...
def effects(metrics):
    ...
    contrasts=[('consensus_given_anchor','anchor_consensus','anchor'),
        ('pairwise_given_consensus','anchor_consensus_both','anchor_consensus'),
        ('median_given_consensus','anchor_consensus_median','anchor_consensus'),
        ('votes_given_consensus','anchor_consensus_votes','anchor_consensus'),
        ('median_given_votes','anchor_consensus_both','anchor_consensus_votes'),
        ('votes_given_median','anchor_consensus_both','anchor_consensus_median')]
    return pd.DataFrame([{'Gender':'M','Season':int(s),'comparison':name,'delta_brier':float(r[a]-r[b])}
        for s,r in wide.iterrows() for name,a,b in contrasts])
...
def evaluate(ctx):
    ...
    for s in SEASONS:
        ...
        for f in wf.CONSENSUS+wf.NEW:
            r=corr.loc[f,wf.BASE+(wf.CONSENSUS if f in wf.NEW else [])].dropna();name=r.abs().idxmax() if len(r) else 'training_constant'
        ...
        for recipe in wf.RECIPES:
            ...
            event('comparison_complete',season=s,recipe=recipe,brier=row['brier'],source=origin,completed=len(rows),total=20)
    ...
    receipt={'new_classifier_fits':int(new),'upstream_replays':int(up),'local_reuses':int(reuse),'total_comparisons':20,'new_rating_fits_in_evaluation':0}
    atomic_json(out/'evaluation_receipt.json',receipt);atomic_json(out/'decisions.json',decision)
    atomic_json(out/'summary.json',dict(status='COMPLETE',phase='men_shared_ranking_matchups',fingerprint=ctx['fingerprint'],**receipt,
        ...
        limitations=['Repeatedly-used 2016–2019 seasons; no untouched-test claim',
          'Compact fixed logistic is not the submitted production recipe',
          'Consensus is an established control, not a novel feature claim',
          'Ordinal systems are correlated; vote fraction is not a win probability or independent evidence count',
          'All legal contemporary systems are used; differs from the older pre-2013 vocabulary',
          'Historical source revisions cannot be ruled out by the provided snapshot',
          'No post-cutoff rankings, future tournament targets, external downloads or new rating fits',
          'Two matchup-level hypotheses, no per-system selection or algorithm search']))
    preservation(ctx);stage_seal(out,'evaluation',EVAL_FILES)


def report(ctx):
    from ranking_plots import render
    out=ctx['directory'];stage_check(out,'evaluation',EVAL_FILES);stage_check(out,'prepare',PREP_FILES)
    html=render(out,ctx['kit']/'evidence/round11');preservation(ctx)
    names=[n for n in EVAL_FILES if n!='predictions.csv']+[n for n in PREP_FILES if n not in ['team_profiles.csv','pair_support.csv']]+['preflight.json','manifest.json','prepare_hashes.json','evaluation_hashes.json']
    if (out/'cache_migration.json').is_file(): names.append('cache_migration.json')
    dest=ctx['kit']/'reports/milestone_12_return.zip';partial=dest.with_suffix('.zip.partial')
    ...
    with zipfile.ZipFile(partial,'w',compression=zipfile.ZIP_DEFLATED) as z:
        ...
        z.write(ctx['prior_zip'],'prior_milestone_11_return.zip')
        z.writestr('return_integrity.json',json.dumps({'sha256':{**{n:sha(out/n) for n in names},
            'prior_milestone_11_return.zip':sha(ctx['prior_zip'])},
    ...
    paths={'kit':kit,'repo':Path(os.environ.get('MARCH_REPO',str(home/'march-machine-learning-mania-2026'))),
        **{k:Path(os.environ.get('MARCH_'+k.upper()+'_KIT',str(home/name))) for k,name in
           [('shooting','march_shooting_research'),('schedule','march_schedule_research'),
            ('temporal','march_temporal_form'),('possession','march_possession_research'),('margin','march_margin_research')]}}
