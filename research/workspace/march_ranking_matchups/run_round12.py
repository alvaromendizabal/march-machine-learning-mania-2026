...
def failure_bundle(kit:Path, stage:str, error:BaseException) -> Path:
    ...
    reports=kit/'reports'
    dest=reports/'milestone_12_failure.zip';tmp=dest.with_suffix('.zip.partial')
    ...
    with zipfile.ZipFile(tmp,'w',compression=zipfile.ZIP_DEFLATED) as z:
        ...
        for name in ['failure.json','latest_run.json',f'ranking_matchups_{stage}.log']:
            p=reports/name
...
def run_stage(stage:str,phase:str='ranking_matchups',max_seconds:int=120) -> None:
    if stage not in ('prepare','evaluate','report') or phase != 'ranking_matchups':
    ...
    with (reports/'execution.lock').open('a') as lock:
        ...
        for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
            env[k]='2'
        command=[sys.executable,'-u',str(kit/'ranking_workflow.py'),stage]
...
if __name__=='__main__':
    ...
    parser.add_argument('--phase',choices=['ranking_matchups'],default='ranking_matchups')
