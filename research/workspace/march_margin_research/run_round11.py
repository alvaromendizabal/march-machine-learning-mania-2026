...
def run_stage(stage:str,phase:str='margin_research',max_seconds:int=120) -> None:
    if stage not in ('prepare','evaluate','report') or phase != 'margin_research':
    ...
    with (reports/'execution.lock').open('a') as lock:
        ...
        for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
            env[k]='2'
        command=[sys.executable,'-u',str(kit/'margin_workflow.py'),stage]
...
if __name__=='__main__':
    ...
    parser.add_argument('--phase',choices=['margin_research'],default='margin_research')
