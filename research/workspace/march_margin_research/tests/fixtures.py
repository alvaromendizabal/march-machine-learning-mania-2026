...
def build_fixture(root):
    root=Path(root);names={'kit':'march_margin_research','repo':'repo','shooting':'march_shooting_research',
        'schedule':'march_schedule_research','temporal':'march_temporal_form','possession':'march_possession_research','win_strength':'march_win_strength'}
    args={k:root/n for k,n in names.items()}
    for p in args.values():p.mkdir(parents=True)
    kit,repo=args['kit'],args['repo']
    for n in w.SOURCES+['11_margin_robustness.ipynb']:
    ...
    expanded=[]
    for row in scores:
        for name in ['anchor','anchor_bt','anchor_bt_uncertainty']:
            expanded.append(dict(row,recipe=name))
    table=pd.DataFrame(expanded)
    w.atomic_csv(kit/'evidence/round10/log_metrics.csv',table)
    import zipfile,hashlib
    payload={'metrics.csv':table.to_csv(index=False,float_format='%.17g').encode(),
             'decisions.json':json.dumps({'synthetic':True}).encode()}
    report=args['win_strength']/'reports/milestone_10_return.zip';report.parent.mkdir(parents=True)
