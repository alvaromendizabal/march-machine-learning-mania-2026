"""Independent rounds 20/21; read-only upstream reuse and bounded saved fits."""
...
SNAP_FILES=['matchups.csv','profiles.csv','coverage.csv','support.csv','diagnostics.csv']
PREP_FILES=['prepare.json','coverage.csv','profiles.csv','feature_registry.csv','prior_replay.csv','label_audit.csv','context_diagnostics.csv']
...
def config(round_id):
    ...
    return {'round':round_id,'validation_seasons':fr.VALIDATION,'snapshot_seasons':fr.YEARS,
       'recipes':fr.recipes(round_id),'parameters':fr.PARAMETERS[round_id],'C':.1,'fit_cap':20,'rating_fit_cap':0,
       'primary':'context_given_controls','genders':['M'],'threads':2,'new_candidate_definitions':4,'unadjusted_rate_controls':4,
...
def preflight(kit,repo,shooting,rankings,consensus,round_id):
    ...
    protected=SOURCES+['constraints.json','run_tests.py']+[str(p.relative_to(kit)) for p in (kit/'evidence').rglob('*') if p.is_file()]
    ...
    atomic_json(out/'preflight.json',{'status':'PASS','state':state,'snapshots_reused':12,'reference_fits_verified':4,
          'new_rating_fit_cap':0,'new_classifier_fit_cap':20,'source_repository_imported':False,'round':round_id})
    event('preflight_pass',round=round_id,snapshots=12,new_rating_fit_cap=0,new_classifier_fit_cap=20)
...
def audit(ctx):
    c=read_csv(ctx['raw']/'MRegularSeasonCompactResults.csv');d=read_csv(ctx['raw']/'MRegularSeasonDetailedResults.csv')
    rows=[]
    for s in fr.YEARS:
        try:
            g=fr.legal_long(c,d,s,ctx['round']);p=fr.profiles(g)
            base,_,pairs=upstream_tables(ctx,s);ids=sorted(set(pairs.Team1ID)|set(pairs.Team2ID))
            require(set(ids)<=set(p.index) and (p.loc[ids,'detailed_games']>=10).all(),'Insufficient seeded detailed-game support')
            if ctx['round']=='21':
                require(np.isfinite(base[['adj_offense','adj_defense']].to_numpy()).all(),'Missing cached opponent adjustment')
            status='PASS';error='';n=len(g)//2
        except ValueError as e:status='STOP';error=str(e);n=0
        rows.append({'Season':s,'status':status,'physical_games':n,'error':error})
        event('all_season_schema_audit',season=s,status=status)
    atomic_csv(ctx['directory']/'data_quality.csv',pd.DataFrame(rows))
    okay=all(r['status']=='PASS' for r in rows)
    atomic_json(ctx['directory']/'quality_audit.json',{'status':'PASS' if okay else 'STOP','models_fitted':0,'fingerprint':ctx['fingerprint'],'audit_sha256':sha(ctx['directory']/'data_quality.csv')})
    preservation(ctx);require(okay,'All-season schema audit failed; inspect data_quality.csv before fitting')

def build_seasons(ctx, seasons):
    out=ctx['directory'];rid=ctx['round']
    audit_record=read_json(safe_file(out,'quality_audit.json'))
    require(audit_record['status']=='PASS' and audit_record['fingerprint']==ctx['fingerprint'] and audit_record['audit_sha256']==sha(out/'data_quality.csv'),'Run/verify the all-season audit before construction')
    new_snapshots=0
    ...
    for i,s in enumerate(seasons):
        ...
        if not checkpoint(folder,SNAP_FILES):
            ...
            event('context_construction_started',round=rid,season=s,completed=i,total=len(seasons))
            built,prof,cov,support,diag=fr.build_snapshot(base,pairs,long,rid)
            for n,f in [('matchups.csv',built),('profiles.csv',prof),('coverage.csv',cov),('support.csv',support),('diagnostics.csv',diag)]:atomic_csv(folder/n,f)
            seal(folder,SNAP_FILES);new_snapshots+=1
        event('snapshot_complete',round=rid,season=s,completed=i+1,total=len(seasons),new_snapshots=new_snapshots,new_rating_fits=0)
    return {'new_rating_fits':0,'new_feature_snapshots':new_snapshots,
...
def prepare(ctx):
    out=ctx['directory'];rid=ctx['round']
    smoke_record=read_json(safe_file(out,'smoke.json'))
    require(smoke_record.get('status')=='PASS_TECHNICAL_SMOKE' and smoke_record.get('new_feature_snapshots')==0,'Run the smoke twice and verify zero new construction before preparation')
    ...
    diagnostics=pd.concat([read_csv(out/f'snapshots/M_{s}/diagnostics.csv') for s in fr.YEARS],ignore_index=True)
    for n,f in [('coverage.csv',coverage),('profiles.csv',profiles),('feature_registry.csv',fr.registry(rid)),
                ('prior_replay.csv',pd.DataFrame(replays)),('label_audit.csv',bundle[3]),('context_diagnostics.csv',diagnostics)]:atomic_csv(out/n,f)
...
def effects_table(metrics,rid):
    ...
    comparisons={'context_given_controls':('both','rates'),'rates_given_reference':('rates','reference'),
...
def decision(effects,rid):
    t=config(rid)['thresholds'];v=effects.query("comparison=='context_given_controls'").sort_values('Season')
    ...
    return {'comparison':'context_given_controls','mean_delta':float(d.mean()),'improved_seasons':int((d<0).sum()),
...
def evaluate(ctx):
    ...
    primary=effects.query("comparison=='context_given_controls'")
    ...
    diagnosed['support_bin']=pd.cut(diagnosed[col],bins=[-1,0,2,5,10,np.inf],labels=['zero','0-2','2-5','5-10','10+'],include_lowest=True)
    ...
    atomic_json(out/'summary.json',dict(status='COMPLETE',fingerprint=ctx['fingerprint'],**receipt,decision=d,
        ...
        'Fixed compact logistic, not the final pooled 128-input XGBoost recipe','Four descriptive controls and four duplicated controls are not novel candidate definitions',
        'Rounds 20 and 21 are independent; no cross-round selection or ensemble',
...
def report(ctx):
    ...
    quality_names=['data_quality.csv','quality_audit.json'] if (out/'quality_audit.json').is_file() else []
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('round',choices=['20','21']);parser.add_argument('stage',choices=['audit','smoke','prepare','evaluate','report']);a=parser.parse_args()
