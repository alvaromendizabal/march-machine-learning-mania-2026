...
class IOTests(unittest.TestCase):
    ...
    def test_gate_does_not_promote_bad_family(self):
        e=pd.DataFrame([{'Season':s,'comparison':c,'delta_brier':.001} for c in ['context_given_controls','both_given_duplicate'] for s in f.VALIDATION])
        self.assertEqual(w.decision(e,'20')['decision'],'DO_NOT_PROMOTE')
    def test_gate_requires_duplicate_control(self):
        e=pd.DataFrame([{'Season':s,'comparison':c,'delta_brier':-.001 if c=='context_given_controls' else .001} for c in ['context_given_controls','both_given_duplicate'] for s in f.VALIDATION])
        self.assertEqual(w.decision(e,'21')['decision'],'DO_NOT_PROMOTE')
    def test_gate_positive_case(self):
        e=pd.DataFrame([{'Season':s,'comparison':c,'delta_brier':-.001} for c in ['context_given_controls','both_given_duplicate'] for s in f.VALIDATION])
        self.assertEqual(w.decision(e,'21')['decision'],'CONSIDER_PRODUCTION_TRANSFER')
    def test_partial_gate_rejected(self):
        e=pd.DataFrame([{'Season':2022,'comparison':'context_given_controls','delta_brier':-.1}])
        with self.assertRaises(ValueError):w.decision(e,'20')
    def test_hard_timeout_preserves_files(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(run_round,'__file__',str(p/'run_round.py')):
                with self.assertRaises(TimeoutError):run_round.run_stage('20','smoke',1)
            self.assertEqual((p/'keep.txt').read_text(),'checkpoint')
            self.assertTrue((p/'reports/round20/milestone_20_failure.zip').is_file())
    def test_invalid_stage_does_not_launch(self):
        with self.assertRaises(ValueError):run_round.run_stage('20','unknown',1)
    def test_invalid_ceiling(self):
        with self.assertRaises(ValueError):run_round.run_stage('20','smoke',900)
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(w.rf,'EXPECTED_SHA',head):
                for rid in ['20','21']:
                    ...
                    # Simulate interruption during a write, before the completion seal.
                    original=w.atomic_csv;counter={'n':0}
                    def interrupt(*a,**kw):
                        counter['n']+=1
                        if counter['n']==2:raise RuntimeError('Intentional synthetic interruption')
                        return original(*a,**kw)
                    with patch.object(w,'atomic_csv',side_effect=interrupt):
                        with self.assertRaises(RuntimeError):w.smoke(ctx)
                    self.assertFalse((ctx['directory']/'snapshots/M_2013/complete.json').exists())
                    w.smoke(ctx)
                    self.assertEqual(w.read_json(ctx['directory']/'smoke.json')['new_rating_fits'],0)
                    w.smoke(ctx)
                    self.assertEqual(w.read_json(ctx['directory']/'smoke.json')['new_feature_snapshots'],0)
                    w.prepare(ctx);self.assertEqual(w.read_json(ctx['directory']/'prepare.json')['new_rating_fits'],0)
                    self.assertEqual(w.read_json(ctx['directory']/'prepare.json')['new_feature_snapshots'],11)
                    ...
                    with patch.object(f,'build_snapshot',side_effect=AssertionError('Should reuse features')),patch.object(w.rf,'fitted_model',side_effect=AssertionError('Should reuse classifiers')):
                    ...
                    w.preservation(ctx)
                bad=ctx['directory']/'snapshots/M_2013/matchups.csv';bad.write_text('corrupt')
