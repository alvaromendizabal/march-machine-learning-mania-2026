        ...
        for rnd in range(24):
            ...
            for a,b in zip(perm[::2],perm[1::2]):
                ...
                for side in ['W','L']:
                    ...
                    for k,v in {'FGM':made,'FGA':60+int(rng.integers(0,7)),'FGM3':threes,'FGA3':int(rng.integers(18,27)),'FTM':ft,'FTA':18,
                                'OR':int(rng.integers(7,12)),'DR':28,'TO':int(rng.integers(8,17)),'Ast':int(rng.integers(12,20)),'Stl':int(rng.integers(3,10)),'Blk':int(rng.integers(1,7))}.items():box[side+k]=v
...
def build_fixture(root):
    root=Path(root);args={k:root/n for k,n in [('kit','rounds_20_21'),('repo','repo'),
    ...
    for name in w.SOURCES+['package_returns.py','run_tests.py','20_pace_and_variance.ipynb','21_opponent_style_responses.ipynb']:
        src=ROOT/name
        if src.exists():
