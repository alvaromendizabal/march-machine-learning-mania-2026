...
def run_stage(stage:str,phase:str='smoke',max_seconds:int=600) -> None:
    if stage not in ('prepare','evaluate','report') or phase not in ('smoke','panel'):
        raise ValueError('Invalid stage or phase')
    if not 1<=max_seconds<=600:
        raise ValueError('Stage limit must be between 1 and 600 seconds')
    ...
    with (reports/'execution.lock').open('a') as lock:
        ...
        for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
            env[k]='2'
        command=[sys.executable,'-u',str(kit/'research_workflow.py'),stage,'--phase',phase]
        ...
        with path.open('ab',buffering=0) as log:
            try:
                while p.poll() is None:
                    ...
                    if now-last>=15:
                        msg=json.dumps({'event':'heartbeat','stage':stage,'phase':phase,
