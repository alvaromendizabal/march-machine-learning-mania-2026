...
def run_stage(stage:str,phase:str='smoke',max_seconds:int=300) -> None:
    if stage not in ('prepare','evaluate','report') or phase != 'smoke':
    ...
    with (reports/'execution.lock').open('a') as lock:
        ...
        for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
            env[k]='2'
        command=[sys.executable,'-u',str(kit/'schedule_workflow.py'),stage]
