...
def build_seasons(ctx, seasons):
    out=ctx['directory'];rid=ctx['round'];new_ratings=0;reused_ratings=0;new_snapshots=0
...
def report(ctx):
    ...
    names=[n for n in EVAL_FILES if n!='predictions.csv']+[n for n in PREP_FILES if n!='profiles.csv']+['preflight.json','manifest.json','prepare_hashes.json','evaluation_hashes.json','executed_tests.json','execution_telemetry.json']
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('round',choices=['16','17']);parser.add_argument('stage',choices=['smoke','prepare','evaluate','report']);a=parser.parse_args()
