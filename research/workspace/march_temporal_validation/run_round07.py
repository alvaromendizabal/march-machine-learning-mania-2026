...
def run_stage(stage:str,phase:str='temporal_validation',max_seconds:int=120) -> None:
    if stage not in ('prepare','replicate','components','report') or phase != 'temporal_validation':
        raise ValueError('Invalid stage or phase')
    limits={'prepare':180,'replicate':120,'components':180,'report':120}
    ...
    with (reports/'execution.lock').open('a') as lock:
        ...
        for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
            env[k]='2'
        command=[sys.executable,'-u',str(kit/'validation_workflow.py'),stage]
...
if __name__=='__main__':
    ...
    parser.add_argument('--phase',choices=['temporal_validation'],default='temporal_validation')
