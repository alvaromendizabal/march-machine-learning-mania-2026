...
BASE = list(cf.ALL)
FAMILIES = {'16': ['steal_turnovers', 'nonsteal_turnovers'],
            '17': ['blocked_shots', 'free_throw_pressure']}
# Sign is the direction of an offensive effect when "higher is favorable".
TARGETS = {
    'steal_turnovers': ('opp_Stl', 'possessions', -1),
    'nonsteal_turnovers': ('nonsteal_TO', 'possessions', -1),
    'blocked_shots': ('opp_Blk', 'two_attempts', -1),
    'free_throw_pressure': ('FTA', 'FGA', 1),
}
...
PARAMETERS = {r: {'cutoff_day': 132, 'alpha': 20.0, 'prior_exposure': 100.0,
                 ...
                 'coefficient_sign': 'higher favorable for offense and defense',
...
QUARANTINE_POLICY = {'version':'turnover-quarantine-20260912',
    'max_season_game_fraction':0.01, 'max_seeded_team_game_fraction':0.05,
    'action':'exclude both orientations from both turnover rate targets only',
    'raw_data_edited':False, 'tournament_games_excluded':False}
PARAMETERS['16']['quality_policy'] = QUARANTINE_POLICY


def turnover_quality(d):
    """Inspect a pre-cutoff detailed table without changing any recorded count."""
    required=['Season','DayNum','WTeamID','LTeamID','WStl','LStl','WTO','LTO']
    require(set(required).issubset(d), 'Missing turnover quality columns')
    values=d[required].to_numpy(dtype=float)
    require(np.isfinite(values).all() and (values>=0).all() and (values==np.floor(values)).all(),
            'Invalid turnover-quality identities/counts')
    bad=d.WStl.gt(d.LTO)|d.LStl.gt(d.WTO)
    total=pd.concat([d.WTeamID,d.LTeamID]).value_counts()
    removed=pd.concat([d.loc[bad,'WTeamID'],d.loc[bad,'LTeamID']]).value_counts()
    team=pd.DataFrame({'TeamID':total.index,'regular_games':total.to_numpy()})
    team['excluded_games']=team.TeamID.map(removed).fillna(0).astype(int)
    team['excluded_fraction']=team.excluded_games/team.regular_games
    rows=d.loc[bad,required].copy()
    rows['reason']='steals_exceed_opposing_turnovers'
    summary={'physical_games':len(d),'excluded_physical_games':int(bad.sum()),
             'excluded_fraction':float(bad.mean()) if len(d) else 0.0}
    return bad, team, rows, summary


def seeded_quality_gate(long,seeded):
    summary=long.attrs.get('turnover_quality')
    if summary is None:return
    team=pd.DataFrame(long.attrs['turnover_team_quality']).set_index('TeamID')
    require(set(seeded).issubset(team.index),'Seeded team absent from quality audit')
    require((team.loc[seeded,'excluded_fraction']<=QUARANTINE_POLICY['max_seeded_team_game_fraction']).all(),
            'More than 5% of a seeded team’s games would be excluded; inspect quality audit')
...
def legal_long(compact: pd.DataFrame, detailed: pd.DataFrame, season: int,
    ...
    box += ['Stl'] if round_id == '16' else ['Blk']
    ...
    quality = None
    if round_id == '16':
        bad, team_quality, excluded_rows, quality = turnover_quality(d)
        require(quality['excluded_fraction'] <= QUARANTINE_POLICY['max_season_game_fraction'],
                'Turnover inconsistencies exceed 1% of season games; inspect data-quality report')
        d = d.loc[~bad].reset_index(drop=True)
        require(not d.empty, 'No valid turnover games remain')
    sides = []
    for own, opp, sign in [('W', 'L', 1), ('L', 'W', -1)]:
        out = pd.DataFrame({'Season': d.Season, 'DayNum': d.DayNum,
            ...
            'home': sign*d.WLoc.map({'H': 1., 'A': -1., 'N': 0.})})
        for b in box:
    ...
    if round_id == '16':
        long['nonsteal_TO'] = long.TO-long.opp_Stl
        require((long.nonsteal_TO >= 0).all(), 'Steals exceed opposing turnovers; do not silently clip')
    if quality is not None:
        long.attrs['turnover_quality'] = quality
        long.attrs['turnover_team_quality'] = team_quality.to_dict('records')
    return long
...
def fit_target(long: pd.DataFrame, target: str, seeded: list[int],
    ...
    Both coefficient sets are sign-oriented after fitting. We fit season-local
...
def registry(round_id: str) -> pd.DataFrame:
    ...
    out['evidence']='prepared hypothesis; keep/reject only after controlled Brier comparisons'
    return out
