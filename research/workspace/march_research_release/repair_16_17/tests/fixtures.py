            ...
            for a,b in zip(perm[::2],perm[1::2]):
                ...
                for side in ['W','L']:
                    ...
                    for k,v in {'FGM':made,'FGA':60,'FGM3':threes,'FGA3':22,'FTM':ft,'FTA':18,
                                'OR':9,'DR':28,'TO':12,'Stl':int(rng.integers(3,10)),'Blk':int(rng.integers(1,7))}.items():box[side+k]=v
...
def build_fixture(root):
    root=Path(root);args={k:root/n for k,n in [('kit','march_feature_rounds_16_17'),('repo','repo'),
    ...
    for name in w.SOURCES+['package_returns.py','run_tests.py','16_turnover_mechanisms.ipynb','17_blocks_and_foul_pressure.ipynb']:
        src=ROOT/name
        if src.exists():
