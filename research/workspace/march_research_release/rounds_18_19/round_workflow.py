...
def audit(ctx):
    ...
    for s in fr.YEARS:
        try:
            g=fr.legal_long(c,d,s,ctx['round']);status='PASS';error=''
            n=len(g)//2
...
def build_seasons(ctx, seasons):
    ...
    require(audit_record['status']=='PASS' and audit_record['fingerprint']==ctx['fingerprint'] and audit_record['audit_sha256']==sha(out/'data_quality.csv'),'Run/verify the all-season audit before fitting')
...
def evaluate(ctx):
    ...
    atomic_json(out/'summary.json',dict(status='COMPLETE',fingerprint=ctx['fingerprint'],**receipt,decision=d,
        ...
        'Rounds 18 and 19 are independent; no cross-round selection or ensemble',
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('round',choices=['18','19']);parser.add_argument('stage',choices=['audit','smoke','prepare','evaluate','report']);a=parser.parse_args()
