...
def run_stage(stage:str,phase:str='replication',max_seconds:int=120) -> None:
    if stage not in ('prepare','replicate','ablate','report') or phase != 'replication':
        raise ValueError('Invalid stage or phase')
    limits={'prepare':300,'replicate':180,'ablate':240,'report':120}
    ...
    with (reports/'execution.lock').open('a') as lock:
        ...
        for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
            env[k]='2'
        command=[sys.executable,'-u',str(kit/'record_workflow.py'),stage]
...
if __name__=='__main__':
    ...
    parser.add_argument('stage',choices=['prepare','replicate','ablate','report'])
    parser.add_argument('--phase',choices=['replication'],default='replication')
