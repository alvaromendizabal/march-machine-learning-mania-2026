...
def failure_bundle(kit:Path, stage:str, error:BaseException) -> Path:
    """Export bounded diagnostics only; never include data, models, or environment secrets."""
    reports=kit/'reports'
    dest=reports/'milestone_13_failure.zip';tmp=dest.with_suffix('.zip.partial')
    ...
    with zipfile.ZipFile(tmp,'w',compression=zipfile.ZIP_DEFLATED) as z:
        ...
        for name in ['failure.json','latest_run.json',f'consensus_validation_{stage}.log']:
            p=reports/name
...
def run_stage(stage:str,phase:str='consensus_validation',max_seconds:int=120) -> None:
    if stage not in ('prepare','evaluate','report') or phase != 'consensus_validation':
    ...
    with (reports/'execution.lock').open('a') as lock:
        ...
        for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS'):
            env[k]='2'
        command=[sys.executable,'-u',str(kit/'consensus_workflow.py'),stage]
        ...
        with path.open('ab',buffering=0) as log:
            try:
                ...
                try:
                    print('Diagnostic archive:', failure_bundle(kit,stage,error),flush=True)
...
if __name__=='__main__':
    ...
    parser.add_argument('--phase',choices=['consensus_validation'],default='consensus_validation')
